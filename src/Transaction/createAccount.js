...
const AddFunds = () => {
  ...
  const handleAddFunds = async (e) => {
    e.preventDefault();
    try {
      const response = await axios.post(`${process.env.REACT_APP_BKD_URI}/api/v3/add-funds`, {
        amount,
  ...
  const handleCaptureFunds = async (e) => {
    e.preventDefault();
    try {
      const response = await axios.post(`${process.env.REACT_APP_BKD_URI}/api/v3/capture-funds`, {
        orderId,
  ...
  return (
    <>
      ...
      <div style={{ display: 'flex' }}>
        <Sidebar />
        <div style={{ flex: 1, marginTop: '2%', margin: '1%', marginLeft: '9%' }}>
          <Box>
            <Grid container>
              <Grid item xs={12} md={6} lg={6}>
                <div style={{ background: "rgb(230, 170, 20)", borderRadius: "5px", height: "400px", display: "flex", flexDirection: "column", justifyContent: "center" }}>
                  <div style={{ margin: "2%" }}>
                    <b>Add Funds</b>
                  </div>
              ...
              <Grid item xs={12} md={6} lg={6}>
                <div style={{ background: "rgb(198, 198, 198)", borderRadius: "5px", height: "400px", display: "flex", flexDirection: "column", justifyContent: "center" }}>
                  {approvalLink && (
                    <div >
