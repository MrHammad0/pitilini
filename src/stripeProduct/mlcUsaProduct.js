...
const SubscriptionForm = () => {
  ...
  const convertIntialValue = async () => {
    try {
      const id = localStorage.getItem('IntialUsaShipId');
      const response = await axios.get(`${process.env.REACT_APP_BKD_URI}/api/v3/create/USA/mlc/shipment/${id}`);
      console.log("USA shipment successfully created", response);
    } catch (error) {
      console.log("USA shipment not created", error);
    }
  };
  ...
  const handleSubmit = async (event) => {
    ...
    try {
      ...
      const response = await axios.post(`${process.env.REACT_APP_BKD_URI}/api/v3/create-mlc-subscription`, {
        email,
          ...
          <Grid item xs={12} md={6} lg={6}>
            <div style={{ width: "70%", marginLeft: "10%" }}>
              <div style={{ display: "flex", alignItems: 'center', justifyContent: "start", marginLeft: "4%" }}>
                ...
                <strong style={{ margin: "1%" }}> MLC a cuentas o tarjetas bancarias</strong>
              </div>
              ...
              <div style={{ display: "flex", justifyContent: "space-between" }}>
                <div style={{ display: "flex", flexDirection: "column", alignItems: "start", marginLeft: "4%" }}>
                  <b>Shipping</b>
                  <p>MLC a cuentas o tarjetas bancarias</p>
                </div>
