...
const SubscriptionForm = () => {
  ...
  const convertIntialValue = async () => {
    try {
      ...
      const response = await axios.get(`${process.env.REACT_APP_BKD_URI}/api/v3/create/USA/USD/shipment/${id}`);
  ...
  const handleSubmit = async (event) => {
    ...
    try {
      ...
      const response = await axios.post(`${process.env.REACT_APP_BKD_URI}/api/v3/create-USD-subscription`, {
        email,
          ...
          <Grid item xs={12} md={6} lg={6}>
            <div style={{ width: "70%", marginLeft: "10%" }}>
              <div style={{ display: "flex", alignItems: 'center', justifyContent: "start", marginLeft: "4%" }}>
                ...
                <strong style={{ margin: "1%" }}> Dólares en La Habana</strong>
              </div>
